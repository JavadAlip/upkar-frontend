  ...
  return (
    <>
      <style>{`
        ...
        .cmplt-navbar { font-family: 'Figtree', sans-serif; }

        @keyframes cmplt-shimmer {
        ...
        .cmplt-contact-btn {
          ...
          animation: cmplt-shimmer 5s ease infinite;
          color: #fff;
        ...
        .cmplt-contact-btn:hover {
          transform: translateY(-1px) scale(1.03);
          box-shadow: 0 5px 18px rgba(45,92,58,0.32);
        }
        .cmplt-contact-btn:active { transform: scale(0.98); }

        .cmplt-contact-btn-mobile {
          background: linear-gradient(
            120deg,
            #2D5C3A 0%, #3a7049 35%, #9eaaa2 50%, #3a7049 65%, #2D5C3A 100%
          );
        ...
        .cmplt-nav-link:hover { color: #2D5C3A; }
        .cmplt-nav-link:hover::after,
        .cmplt-nav-link.active::after { width: 100%; }
        .cmplt-nav-link.active { color: #2D5C3A; font-weight: 500; }
        ...
        .cmplt-mobile-nav-link:hover {
          padding-left: 5px;
          color: #2D5C3A !important;
        }
      `}</style>

      <nav className="cmplt-navbar fixed top-0 left-0 w-full bg-white/40 backdrop-blur-md shadow-md p-4 flex items-center justify-between z-50">
        {/* LEFT — Logo */}
        <Link to="/">
          <img
            src={logo}
        ...
        <ul className="hidden md:flex absolute left-1/2 transform -translate-x-1/2 space-x-8 items-center">
          {navLinks.map((item) => (
            <li key={item.name}>
              <Link
                to={item.path}
                className={`cmplt-nav-link ${isActive(item.path) ? 'active' : ''}`}
              >
                {item.name}
              </Link>
            </li>
        ...
        <div className="flex items-center gap-3">
          <Link to="/contact" className="hidden md:block">
            <button className="cmplt-contact-btn">Contact Us</button>
          </Link>
          <button
            className="md:hidden text-gray-700 focus:outline-none"
            onClick={() => setMenuOpen(!menuOpen)}
      ...
      {/* MOBILE DRAWER */}
      <div
        className={`cmplt-mobile-drawer cmplt-navbar md:hidden fixed top-[88px] left-0 w-full bg-white shadow-md px-5 pb-5 pt-2 z-[999] ${menuOpen ? 'open' : ''}`}
      >
        <ul className="flex flex-col">
          {navLinks.map((item) => (
            <li key={item.name}>
              <Link
                to={item.path}
                className="cmplt-mobile-nav-link"
                onClick={() => setMenuOpen(false)}
              >
                {item.name}
              </Link>
          ...
          <li className="pt-4">
            <Link to="/contact" onClick={() => setMenuOpen(false)}>
              <span className="cmplt-contact-btn-mobile">Contact Us</span>
            </Link>
          </li>
        </ul>
      </div>
