...
import { createUpcomingProjectList } from "../../../Api";
...
const UpcomingProjectAdd = ({ isOpen, onClose, onAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await createUpcomingProjectList(fd, token);
      onAdded();
      onClose();
    } catch (error) {
      console.error(error);
      ...
      <div className="bg-white p-6 rounded shadow-md w-full max-w-md">
        ...
        <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="px-4 py-2 bg-orange-500 text-white rounded">{loading ? "Adding..." : "Add"}</button>
          </div>
        </form>
      </div>
    </div>
