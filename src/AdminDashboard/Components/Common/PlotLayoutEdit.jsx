...
const PlotLayoutEdit = ({ isOpen, onClose, layout, onUpdate }) => {
  ...
  const [mainImagePreview, setMainImagePreview] = useState(""); // preview or existing URL
  ...
  const addIconField = () => {
    if (icons.length < 10) setIcons([...icons, { file: null, preview: "", heading: "", subheading: "" }]);
  };
  ...
  try {
    ...
    const res = await createPlotLayout(formData, token); // <- pass token
      ...
      <div className="bg-white p-6 rounded shadow-md w-full max-w-lg my-6">
        ...
        <div className="flex flex-col gap-3">
          <div>
            ...
            <input type="file" className="border p-2 w-full rounded" onChange={(e) => { setMainImage(e.target.files[0]); setMainImagePreview(URL.createObjectURL(e.target.files[0])); }} />
          </div>
          ...
          <button type="button" className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300 w-max mb-3" onClick={addIconField}>
            + Add Icon
          </button>
