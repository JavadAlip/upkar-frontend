  ...
  const VisionMissionAdd = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      if (!description || !missionText || !visionText || !image) {
            ...
            <div>
              ...
              {stats.map((stat, index) => (
                <div key={index} className="flex gap-2 mb-2">
                  ...
                  />
                  {stats.length > 1 && (
                    <button type="button" onClick={() => removeStatField(index)} className="text-red-500">
                      Remove
                    </button>
                  )}
                </div>
              ))}
              <button type="button" onClick={addStatField} className="text-blue-500 mb-2">
                + Add Stat
              </button>
            </div>
