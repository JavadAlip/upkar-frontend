...
const ProjectAdd = ({ isOpen, onClose, onProjectAdded }) => {
  ...
  const removeBullet = (index) => setBulletPoints(bulletPoints.filter((_, i) => i !== index));

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!type || !heading || !description || !boxMessage || !image) return alert("All fields required!");
    ...
    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-40 z-50 p-4">
      <div className="bg-white p-6 rounded w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form className="space-y-3" onSubmit={handleSubmit}>
          ...
          <input type="text" placeholder="Heading" value={heading} onChange={(e) => setHeading(e.target.value)} className="border p-2 w-full rounded" />
          ...
          <div>
            ...
            {bulletPoints.map((bp, i) => (
              <div key={i} className="flex gap-2 mb-1 items-center">
                ...
                />
                {bulletPoints.length > 1 && <button type="button" onClick={() => removeBullet(i)} className="text-red-500">Remove</button>}
              </div>
            ))}
            <button type="button" onClick={addBullet} className="text-blue-500 mb-2">+ Add Bullet</button>
          </div>

          <input type="text" placeholder="Box Message" value={boxMessage} onChange={(e) => setBoxMessage(e.target.value)} className="border p-2 w-full rounded" />
          <input type="file" accept="image/*" onChange={(e) => setImage(e.target.files[0])} className="mt-2" />

          <div className="flex justify-end gap-2 mt-4">
            <button type="button" onClick={onClose} className="px-4 py-2 border rounded">Cancel</button>
            <button type="submit" disabled={loading} className="px-4 py-2 bg-orange-500 text-white rounded">{loading ? "Adding..." : "Add"}</button>
          </div>
        </form>
      </div>
    </div>
